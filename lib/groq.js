...
export const getAssetSignal = async (userId, symbol = 'EURUSD') => {
  ...
  const technicalScore = c30.length >= 12
    ? Number(getMasterTechnicalScore(c30, Number(price || 0), Number(isXAU ? 0 : spread))?.score || 0)
    : null;
  ...
  const xauSessionOk = (istMins >= 13 * 60 + 30 && istMins <= 17 * 60 + 30) || (istMins >= 18 * 60 + 30 && istMins <= 22 * 60 + 30);
  ...
  if (!baseDataReady) {
    ...
    waitReason = 'Gold is outside London/NY high-liquidity windows.';
  ...
  const slDist = isXAU ? Math.min(150, Math.max(80, Math.round(Math.abs(trendPct) * 100))) : 25;
  ...
  const confidence = signal === 'WAIT' ? 'LOW' : (confidencePct >= 75 ? 'HIGH' : confidencePct >= 55 ? 'MEDIUM' : 'LOW');
  ...
  const confidenceReason = signal === 'WAIT'
    ...
    : `${confidence} because trend and momentum are aligned with key levels${nextDanger ? ' and event risk is manageable.' : '.'}`;
  ...
  if (isXAU) {
    lines.push(
      `- SL distance: $${slDist} dollar move x 0.01 lot = $${(slDist * unitValue).toFixed(2)} risk`,
      `- TP distance: $${tpDist} dollar move x 0.01 lot = $${(tpDist * unitValue).toFixed(2)} reward`,
    ...
    lines.push(
      `- SL distance: ${slDist} pips x $0.10 unit value = $${(slDist * unitValue).toFixed(2)} risk dollars`,
      `- TP distance: ${tpDist} pips x $0.10 unit value = $${(tpDist * unitValue).toFixed(2)} reward dollars`,
      `- R:R ratio: $${(tpDist * unitValue).toFixed(2)} / $${(slDist * unitValue).toFixed(2)} = ${rr.toFixed(1)}:1`,
      '',
      `Valid Entry Range: ${entryLow.toFixed(decimals)} - ${entryHigh.toFixed(decimals)}`,
      '→ Only tap Buy/Sell in MT5 if current price is within this range.',
