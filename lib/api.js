...
const API_CONFIG = {
  PRIMARY_FOREX_BASE: 'https://open.er-api.com/v6/latest',
  BACKUP_FOREX_BASE: 'https://api.frankfurter.dev/v1/latest',
...
const fetchPrimaryPair = async (pair) => {
  ...
  // Real-time feed for EUR/USD via Binance EURUSDT.
  if (base === 'EUR' && quote === 'USD') {
    ...
    if (!priceRes.ok) throw new Error(`Primary EURUSD API failed (${priceRes.status})`);
    const tick = await priceRes.json();
    const mid = Number(tick?.price || 0);
    if (!mid) return null;
  ...
  const url = `${API_CONFIG.PRIMARY_FOREX_BASE}/${base}`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`Primary price API failed (${res.status})`);
  const json = await res.json();
  const mid = Number(json?.rates?.[quote] || 0);
  if (!mid) return null;
  const spread = quote === 'JPY' ? 0.02 : 0.00013;
  return {
    pair,
    bid: Number((mid - spread / 2).toFixed(quote === 'JPY' ? 3 : 5)),
    ask: Number((mid + spread / 2).toFixed(quote === 'JPY' ? 3 : 5)),
    spread: quote === 'JPY' ? Number((spread * 100).toFixed(1)) : toPips(spread),
    change: 0,
    changePct: 0,
...
const fetchBackupPair = async (pair) => {
  const { base, quote } = pairLabelToCodes(pair);
  const url = `${API_CONFIG.BACKUP_FOREX_BASE}?base=${base}&symbols=${quote}`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`Backup price API failed (${res.status})`);
  const json = await res.json();
  const mid = Number(json?.rates?.[quote] || 0);
  if (!mid) return null;
  const spread = quote === 'JPY' ? 0.02 : 0.00016;
  return {
    pair,
    bid: Number((mid - spread / 2).toFixed(quote === 'JPY' ? 3 : 5)),
    ask: Number((mid + spread / 2).toFixed(quote === 'JPY' ? 3 : 5)),
...
export const getEconomicCalendar = async () => {
  ...
  const mapped = events
    ...
    .filter((event) => ['USD', 'EUR'].includes(event.country));
...
export const getCurrentTradingSession = () => {
  ...
  if (total >= 22 * 60 + 30 || total < 3 * 60 + 30) {
    return {
      session: 'ny',
      label: '🟠 NY Session',
      color: C.orange,
      advice: 'Lower quality than overlap. Only take top setups.',
      minutesRemaining: total >= 22 * 60 + 30 ? (24 * 60 - total) + (3 * 60 + 30) : (3 * 60 + 30 - total),
    };
  }
  return {
    session: 'dead',
